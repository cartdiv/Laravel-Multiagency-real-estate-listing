					...
					<div class="col-md-12 grid-margin stretch-card">
						<div class="card">
							<div class="card-body">
								...
								<form id="myForm" method="POST" action="{{route('update.agency.agent')}}" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $editData->id }}">
                                    <input type="hidden" name="old_img" value="{{ $editData->agent_image }}">
									<div class="form-group mb-3">
										<label for="exampleInputText1" class="form-label">Agent Name</label>
										<input type="text" class="form-control" name="agent_name" value="{{ $editData->agent_name }}" >
									</div>

                                    <div class="form-group mb-3">
										<label for="exampleInputText1" class="form-label">Agent Title</label>
										<input type="text" class="form-control" name="agent_title" value="{{ $editData->agent_title }}">
									</div>

                                    <div class="form-group mb-3">
										<label for="exampleInputText1" class="form-label">Agent Email</label>
										<input type="text" class="form-control" name="agent_email" value="{{ $editData->agent_email }}">
									</div>
                                    <div class="mb-3">
                                        ...
                                        <div class="col-sm-9 text-secondary">
                                             <img id="showImage" src="{{ asset($editData->agent_image) }}" alt="Image" style="width:100px; height: 100px;"  >
                                        </div>

                                    <div class="form-group mb-3">
										<label for="exampleInputText1" class="form-label">Agent phone</label>
										<input type="text" class="form-control" name="agent_number" value="{{ $editData->agent_number }}">
									</div>

                                    <div class="form-group mb-3">
										<label for="exampleInputText1" class="form-label">Agent Description</label>
										<textarea id="maxlength-textarea" class="form-control" name="agent_description" rows="8" placeholder="This textarea has a limit of 100 chars.">{{ $editData->agent_description }}</textarea>
									</div>

                                    <div class="form-group mb-3">
										<label for="exampleInputText1" class="form-label"> Skype (optional)</label>
										<input type="text" class="form-control" name="agent_skype" value="{{ $editData->agent_skype }}">
									</div>
